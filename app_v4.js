...
function getRowCalculations(data, detail, periodPaid, employeeIsRendement) {
    ...
    if (data.rendementActive && data.arrivee) {
        ...
        if (data.pause && data.reprise && data.fin) {
            ...
            if (totalMinutes === 0) {
                // Aucun intervalle standard → RENDEMENT pur (0h payées)
                if (data.fin) {
                    legalDayMinutes = Math.max(0, timeToMinutes(data.fin) - timeToMinutes(data.arrivee));
                } else {
                    legalDayMinutes = 480; // Arrivée seule → journée complète en présence
                }
                totalMinutes = 0; // 0h payées
                legalNightMinutes = 0;
            } else {
                legalDayMinutes = totalMinutes;
                legalNightMinutes = 0;
        ...
        legalDayMinutes = 480;
        legalNightMinutes = 0;
    } else if (typeof periodPaid === 'number' && periodPaid > 0 || data.status === "accident" || data.status === "conge" || data.status === "malade" || data.status === "faute_entreprise") {
        totalMinutes = typeof periodPaid === 'number' && periodPaid > 0 ? periodPaid : 480;
        legalDayMinutes = totalMinutes;
        legalNightMinutes = 0;
...
function generateTable() {
    ...
    const groupPointingBtn = document.getElementById("open-group-pointing-btn");
    if (groupPointingBtn) groupPointingBtn.style.display = isMonthClosed ? "none" : "flex";
    ...
    for (let day = 1; day <= daysCount; day++) {
        ...
        const totalMinutes = calcs.totalMinutes + (paidAbsenceExtra > 0 && (data.arrivee || data.nuitActive) && !data.rendementActive ? paidAbsenceExtra : 0) + recoverExtra;
        ...
        // Si le jour tombe dans une période d'absence et que la période est payée
        // et que le totalMinutes > 0, on considère ce jour comme travaillé
        if (periodPaid > 0 && totalMinutes > 0 && isWorkingDay) {
            paidUnworkedDays++;
...
function updateSummaryKPIs(dayMin, nightMin, workedDays, totalMin, totalWorkingDays) {
    ...
    if (isRdt) {
        ...
        document.getElementById("kpi-total-hours").innerHTML = `<span style="color:#b45309; font-weight:800; font-size:1.1rem;">${rdtDays}j de rendement + ${minutesToHoursStr(totalMin)}</span> <span style="font-size:0.85em;font-weight:800;color:#ea580c;background:#fff7ed;padding:2px 6px;border-radius:6px;border:1px solid #fdba74;display:inline-block;white-space:nowrap;margin-left:4px;">${minutesToDecimal(totalMin)}</span>`;
    } else {
...
function generateRecapTable() {
    ...
    activeEmployees.forEach(emp => {
        ...
        for (let day = 1; day <= daysCount; day++) {
            ...
            if (emp && emp.isRendement && data.pause) {
                hasHourlyData = true;
                hasPause = true;
        ...
        // Vérifier si l'employé a des pauses saisies (pour affichage RENDEMENT + h)
        const empHasPause = emp.isRendement && hasPause;
        ...
        tr.innerHTML = `
            ...
            ${emp && emp.isRendement && !hasPause && workedDays > 0 ?
                ...
                `${emp && emp.isRendement && hasPause ?
                    ...
                    <td style="padding: 12px 16px; text-align:center; font-weight:700; color:#b45309; background:#fffbeb; border:1px solid #fde68a;">RENDEMENT + ${minutesToHoursStr(finalTotalHours)}</td>` :
                    `<td style="padding: 12px 16px; text-align:center;">${minutesToHoursStr(totalJourM)}</td>
                    <td style="padding: 12px 16px; text-align:center;">${minutesToHoursStr(totalNuitM)}</td>
...
if (data.rendementActive && (data.pause || (data.reprise && data.fin) || paidAbsenceExtra > 0 || recoverExtra > 0)) {
        ...
        trElement.querySelector(".val-total-global").innerHTML = `RENDEMENT + ${minutesToHoursStr(totalMinutes)}`;
    ...
    } else if (data.rendementActive && (data.arrivee || data.status === "present")) {
    ...
    employeeIds.forEach((empId, empIndex) => {
        ...
        for (let day = 1; day <= daysCount; day++) {
            ...
            if (mpMode) {
                ...
                totalMinutes = typeof periodPaid === 'number' && periodPaid > 0 ? periodPaid : 480; legalDay = totalMinutes;
            } else if (data.status === "permission_payee") {
                totalMinutes = 480; legalDay = 480;
...
function applyAbsencePeriodToPointage(period, empId) {
    // Pointer automatiquement les jours de la période dans state.pointages + dayDetails
    if (!empId) empId = state.activeEmployeeId;
    if (!empId) return;
    if (!state.pointages) state.pointages = {};
    if (!state.pointages[empId]) state.pointages[empId] = {};
...
function removeAbsencePeriodFromPointage(period, empId) {
...
function createRattrapageDebt(period, empId) {
...
function removeRattrapageDebt(periodId, empId) {
    if (!empId) empId = state.activeEmployeeId;
    if (!empId) return;
...
function getSelectedEmployeeIds() {
    const cbs = document.querySelectorAll(".absence-emp-cb");
    if (cbs.length === 0) return [state.activeEmployeeId];
    const ids = Array.from(cbs).filter(cb => cb.checked).map(cb => cb.value);
    return ids.length > 0 ? ids : [state.activeEmployeeId];
}
...
function renderEmployeeCheckboxes(editingPeriodId) {
    ...
    if (editingPeriodId && state.absencePeriods) {
        Object.keys(state.absencePeriods).forEach(empId => {
            const periods = state.absencePeriods[empId];
            if (Array.isArray(periods)) {
                periods.forEach(p => {
                    if (p.id === editingPeriodId) {
                        preSelected.add(empId);
                    }
                });
            }
        });
    }
    const hasPreSelection = preSelected.size > 0;

    const groups = {};
    employees.forEach(emp => {
        const role = (emp.role || "Autre").trim();
        ...
        <div style="margin-bottom:10px;">
            ...
            <div style="border:1px solid var(--border-color);border-radius:6px;background:var(--bg-card);max-height:180px;overflow-y:auto;">
                ${sortKeys.map(role => {
                    ...
                    const allChecked = emps.every(e => hasPreSelection ? preSelected.has(e.id) : true);
                    const groupKey = role.replace(/[^a-zA-Z0-9]/g, "_");
                    return `
                        <div style="border-bottom:1px solid var(--border-color);padding:4px 6px;">
                            <label style="display:flex;align-items:center;gap:6px;font-size:0.75rem;cursor:pointer;font-weight:600;color:var(--text-primary);">
                                <input type="checkbox" class="sel-all-func" data-func="${role}" ${allChecked ? 'checked' : ''} style="accent-color:var(--accent-primary);">
                                <span>${role}</span>
                                <span style="color:var(--text-muted);font-size:0.65rem;">(${emps.length})</span>
                            </label>
    ...
    document.querySelectorAll(".sel-all-func").forEach(cb => {
    ...
    if (btn) {
        btn.addEventListener("click", () => {
            ...
            empIds.forEach(empId => {
                if (!state.absencePeriods[empId]) state.absencePeriods[empId] = [];
                state.absencePeriods[empId].push(newPeriod);
                // Auto-pointer les jours dans l'onglet Pointage + Remarques
                applyAbsencePeriodToPointage(newPeriod, empId);
                // Créer la dette de rattrapage si coché
                if (isRecover) createRattrapageDebt(newPeriod, empId);
            });
...
function renderAbsencePeriods() {
    ...
    periods.forEach((p, index) => {
        ...
        if (p.minutesPerDay) tagsHtml += `<span style="background:#e0e7ff; color:#4338ca; padding:2px 4px; border-radius:4px; font-size:0.6rem; margin-left:2px;">${p.minutesPerDay/60}h/j</span>`;
    ...
    document.querySelectorAll(".edit-absence-btn").forEach(btn => {
        btn.addEventListener("click", function() {
            ...
            const hpdEl = document.getElementById("absence-hours-per-day");
            if (hpdEl) hpdEl.value = p.minutesPerDay ? (p.minutesPerDay / 60) : 8;
            
            const empSel = document.getElementById("absence-employees-selector");
