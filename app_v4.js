...
function setupEventListeners() {
    ...
    if (bulkCustomPointingBtn && bulkCustomPointingModal) {
        bulkCustomPointingBtn.addEventListener("click", () => {
            ...
            document.getElementById("bulk-custom-pointing-count-label").textContent = 
                `Saisissez les horaires pour les ${selectedCount} employé(s) sélectionné(s).`;
            
            // Default to today if in current month
            ...
            const arr = document.getElementById("bulk-pointing-arr");
            const pau = document.getElementById("bulk-pointing-pau");
            const rep = document.getElementById("bulk-pointing-rep");
            const fin = document.getElementById("bulk-pointing-fin");
            
            const narr = document.getElementById("bulk-pointing-narr");
            const npau = document.getElementById("bulk-pointing-npau");
            const nrep = document.getElementById("bulk-pointing-nrep");
            const nfin = document.getElementById("bulk-pointing-nfin");
            
            arr.value = ""; pau.value = ""; rep.value = ""; fin.value = "";
            narr.value = ""; npau.value = ""; nrep.value = ""; nfin.value = "";
            
            arr.disabled = false; pau.disabled = false; rep.disabled = false; fin.disabled = false;
            document.getElementById("bulk-pointing-night-container").style.display = "none";
            
            bulkCustomPointingModal.classList.add("active");
        });
        
        // Listener absence toggle
        const absenceCb = document.getElementById("bulk-pointing-absence-cb");
        absenceCb.addEventListener("change", (e) => {
            const isAbsent = e.target.checked;
            const container = document.getElementById("bulk-pointing-hours-container");
            container.style.opacity = isAbsent ? "0.3" : "1";
            container.style.pointerEvents = isAbsent ? "none" : "auto";
        });
        
        // Listener night toggle
        const nightCb = document.getElementById("bulk-pointing-night-cb");
        nightCb.addEventListener("change", (e) => {
            document.getElementById("bulk-pointing-night-container").style.display = e.target.checked ? "grid" : "none";
        });
        
        // Auto-format sur les inputs
        const timeInputs = bulkCustomPointingModal.querySelectorAll(".time-input");
        ...
        // Confirm Button
        const confirmBtn = document.getElementById("confirm-bulk-custom-pointing-btn");
        confirmBtn.addEventListener("click", () => {
            ...
            if (isNaN(dayVal) || dayVal < 1 || dayVal > daysCount) {
                alert("Veuillez entrer un jour valide pour le mois en cours (1 à " + daysCount + ").");
                return;
            }
            ...
            selectedIds.forEach(empId => {
                ...
                // Si l'utilisateur n'est pas Admin, on ne peut pas écraser un jour déjà pointé!
                const existingData = state.pointages[empId][dateKey] || {};
                const hasExisting = (existingData.arrivee || existingData.pause || existingData.reprise || existingData.fin || existingData.nuitDebut);
                
                if (hasExisting && !isAdmin) {
                    // On skip cet employé car non-admin
                    return;
                }
                ...
                if (isAbsent) {
                    ...
                    state.pointages[empId][dateKey] = {
                        arrivee: arr, pause: pau, reprise: rep, fin: fin,
                        nuitActive: isNight,
                        nuitDebut: isNight ? narr : "",
                        nuitPause: isNight ? npau : "",
                        nuitReprise: isNight ? nrep : "",
                        nuitFin: isNight ? nfin : "",
                        status: "present",
            ...
            if (applyCount < selectedIds.length) {
                alert(`${applyCount} employé(s) mis à jour.\n${selectedIds.length - applyCount} employé(s) ignoré(s) car ils avaient déjà un pointage et vous n'êtes pas Administrateur.`);
            } else {
                alert(`Pointage appliqué avec succès pour ${applyCount} employé(s).`);
            }
            
            bulkCustomPointingModal.classList.remove("active");
            document.querySelectorAll(".emp-select-cb:checked").forEach(cb => cb.checked = false);
            toggleBulkActionBar();
        });
    }
